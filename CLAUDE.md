...
- `--prune` deletes what the board holds and the repo does not, through
  `GET /edit?func=delete&path=/<name>` (the same handler that lists). Opt-in,
  and it spares only `cfg.json`, `wsec.json`, `presets.json` and
  `2d-gaps.json` — a hand-uploaded ledmap or palette goes with the stale GIFs.
  The keep-list is read off disk, not off the flags, so `-c --prune` cannot
  mistake the GIFs for strangers. `-v --prune` prints the list without acting.
