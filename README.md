...
**Firmware caveat — unresolved, check this on the panel.** WLED documents `1`
as a regular pixel and `0` as one that is never painted, and the notes this
pipeline arrived with say WLED 16.0.1 acts on the *inverse* of that (painting
the `0`s), presumed a firmware bug. The invocation above is written for that
inverse reading: it puts `0` on the logo shape and `1` on the 16% of the panel
around it.

Reading 16.0.1's source does not bear that out. `setUpMatrix()` in
`WLED/wled00/FX_2Dfcn.cpp` maps a pixel — which is what makes it paintable —
only where the gap entry is `> 0`:
...
Taken literally, the file above lights the ground and blanks the logo — the
opposite of what the mask wants. One of the two readings is wrong, and only the
panel can say which: the note came from watching real LEDs, the code from
reading the pinned release. The file is left as the panel-tested invocation
produced it. **If the panel comes up inverted, add `-n/--negative` to the
`$(GAPFILE)` recipe in `gfx/Makefile`** and re-provision — that is the whole
fix, either way.
